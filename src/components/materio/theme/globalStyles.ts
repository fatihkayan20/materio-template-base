...
type ReturnType = {
  'body[style^="padding-right"] .layout-navbar-and-nav-container::after': SxProps;
  ".demo-space-x > *": SxProps;
  ".demo-space-y > *:not(:last-of-type)": SxProps;
  ".MuiGrid-container.match-height .MuiCard-root": SxProps;
  ".ps__rail-y": SxProps;

  "#nprogress": SxProps;
};

const GlobalStyles = (theme: Theme, settings: Settings): ReturnType => {
  const { skin } = settings;
  const perfectScrollbarThumbBgColor = (): string => {
